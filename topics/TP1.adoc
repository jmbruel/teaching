...
|======================
| {pre}		|
1.	Je connais les concepts principaux de {scrum} (vus en cours et au 1er TD)
...
== icon:gitlab[] Gitlab  (icon:github[] GitHub en plan B)
...
=====
+
. icon:users[] Découvrez l'environnement, les menus et configurez votre profil. Observez les différences avec GitHub que vous connaissez déjà.
...
. icon:user-md[] Le Scrum Master créé un dépôt respectant la nomenclature : `MPA2021-G[1|2|3|4][1|2|3|4|5|6|7|8]`.
...
Product Owner icon:male[] (le prof de TD/TP), ainsi que {jmb} (utilisateur `jmb`, en tant que Product Owner).
...
(Makefile, {ant}, {maven}, {gradle}, etc.), si bien sûr les vacances n'ont pas complètement effacé ce que vous
avez fait en S2...
